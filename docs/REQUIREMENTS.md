...
In this challenge you’ll start with exactly that fragile script, and then rebuild it robustly. You’ll turn it into a declarative GitHub Actions workflow with retries, event triggers and concurrency control. You’ll teach it to handle large pull requests, to run real static analysis tools instead of guessing, and finally to go and look things up for itself.
...
You’ll also want GitHub Actions available on the repository you control. You won’t need a local workflow runner for the core challenge, though tools like `act` can be useful later if you want to rehearse jobs locally.
...
Create a workflow file in `.github/workflows`, something like `pr-review.yml`.

Now build your first workflow. It should take the repository owner, the repository name and the pull request number as `workflow_dispatch` inputs, so you can type them into the UI and hit execute. It should fetch the pull request’s changed files, and it should log what came back. You can do this with a shell step that calls the GitHub API or with `actions/github-script`.
...
Don’t add the model yet. This step is about getting comfortable with the shape of a workflow, and about the fact that you can now see every run, its inputs and its logs, in the Actions UI. That’s the first item off your Step 1 list gone.

**Testing**: Execute the workflow from the UI with your fork’s owner, name and pull request number. Open the run in the UI, click through to the step logs, and confirm you can see the diff for each changed file and the pull request title. Execute it a second time and confirm both runs appear in the Actions history with their inputs recorded.

## Step 3

In this step your goal is to turn the diff into an actual review.

Add a step that sends the diff and the pull request metadata to a language model. You can do this with a small script in the repository or an inline step that calls the provider’s API, so you can point it at whichever provider you set up in Step Zero.
...
The second is the guidelines. Take them as a workflow input rather than burying them in the prompt. Your team’s coding standards will change, and the people who want to change them are often not the people who are comfortable editing a workflow. Making the rules an input rather than part of the code is a small decision that pays off every time someone asks for a new rule.

**Testing**: Run the workflow against the pull request. The review should come back in the structure you asked for, with the Stripe key tagged CRITICAL and the nesting in calculate_discount showing up as at least a MINOR issue.

Now change the guidelines input without touching the workflow. Remove the rule about nesting and add one demanding that every function has a docstring. Run it again. The nesting complaint should disappear, and you should not have edited a single line of the workflow to make that happen.

## Step 4

In this step your goal is to close the loop and get the review onto the pull request.

Add a final step that posts the review as a comment, using the GitHub issues comments endpoint. Pull requests are issues as far as that part of the API is concerned, so the URL is /repos/{owner}/{repo}/issues/{number}/comments.

Finally, stop passing your tokens around in plain sight. Store them in GitHub Actions secrets and read them through the workflow environment. The built-in `GITHUB_TOKEN` is usually enough for reading pull requests and writing comments in the same repository, but keep a personal access token available if you need cross-repository access or broader permissions.
...
**Testing**: Run the workflow and check the pull request on GitHub. The review should be there as a comment, readable, with its headings intact.

Then test the quiet path. Run the workflow against the second pull request, the one that fixes the spelling mistake in a comment. Nothing should be posted, and the job log should tell you why it chose not to. If your reviewer manages to find a CRITICAL issue in a corrected spelling, your prompt is being far too eager and that will only get worse from here.

## Step 5

In this step your goal is to make the choice of model a detail rather than a decision.

Right now your workflow is welded to one provider. Change the provider in your completion step to a different one, Anthropic or Google Gemini or OpenAI, whichever you didn’t use in Step 3, and run the same pull request through again. You’ll need a second API key, and you should not need to touch anything else in the workflow. Three lines change. The fetching, the prompt, the posting and the skip logic all stay exactly as they are.

Then do something more interesting. Point it at a model running via Ollama and run the review again with no outbound calls to any commercial API at all. If you want to keep the runner GitHub-hosted, that probably means exposing the model through a reachable endpoint; if you want everything local, use a self-hosted runner. This is the version that can run inside a bank or a defence contractor, and being able to switch to it by editing three lines is the whole argument for keeping your orchestration separate from your model.
...
**Testing**: Run the identical workflow against the same pull request with at least three different providers, including a local model. All three should produce a usable review that catches the Stripe key.
...
Replace manual execution with a `pull_request` trigger so that opening a pull request runs the review automatically.

If you want to test the full workflow against a local-only environment, a self-hosted runner is the cleanest option. Otherwise a standard GitHub-hosted runner is enough for this challenge.

Two bits of care are needed here. GitHub emits a lot of pull request activity, so your workflow should act on `opened`, `reopened` and `synchronize`, and quietly ignore everything else. Somebody adding a label should not trigger a code review.

The second is more subtle, and it’s the reason this step and the next are separate. `synchronize` fires on every single push. If someone pushes three quick fixes in a row, you’ll get three reviews running at once, all posting comments, at least two of them already out of date by the time they land. Set a `concurrency` group on the workflow so that only one review of a given pull request runs at a time, and cancel in-progress runs for stale commits. Cancelling them is usually right for code review, because a review of the final state is worth more than three reviews of states nobody will ever look at again.
...
Then try to break it. Push three commits in quick succession. You should end up with one review, not three. Check the workflow run list and confirm you can see the ones that were cancelled and why.
...
Add retries to the steps that talk to the outside world. Both the GitHub API and every model provider will rate limit you eventually, and a single 429 should not mean a lost review. Three attempts with an increasing delay between them handles the overwhelming majority of real failures.

Add a timeout to the model call. Providers occasionally accept a request and then simply never answer, and without a timeout your workflow run will sit there indefinitely looking like it’s working.

Then handle the case where it genuinely fails anyway. Add error handling to the workflow that posts a short notice to Slack or Discord when a review fails, including the workflow run ID and run URL so you can go straight to the logs. This is the difference between a system you trust and a system you have to check on. Without it, a broken review looks exactly the same as a pull request nobody bothered to review, and you won’t find out for weeks.

**Testing**: Set your model API key to an invalid value and push a commit. The workflow should retry, fail cleanly, and you should get a notification containing the workflow run ID. Follow that ID back to the logs and confirm you can see exactly what went wrong.

Restore the key and confirm normal service resumes. If you can, test the retry path properly by pointing a step at an endpoint that returns a 429 and watching the attempts in the logs with the delay growing between them.

## Step 8

In this step your goal is to handle a pull request that is too big to review in one go.

Right now you send the entire diff to the model as a single prompt. That works fine for the small pull requests you’ve been testing with and falls apart on a real one. Change your workflow so that it reviews each changed file separately and in parallel, rather than concatenating everything into one enormous prompt.
...
Check the job and step view in the Actions UI. You should be able to see the per file reviews running side by side rather than one after another. Compare the total time to what it took in Step 4 and note the difference.
...
Run that tool inside a container rather than assuming it is installed on the runner. GitHub Actions supports containerized jobs and Docker-based actions for exactly this: the linter, its version and its dependencies live in an image, and your workflow stays clean. This also means your reviewer can handle a repository in a language nobody on your platform team has installed on the default runner.
...
This is the longest step in the challenge, mostly because getting a repository checked out and a containerized tool running takes more fiddling than the rest. Give yourself a bit of time.
...
- Split the workflow into a reusable workflow that any repository in your organisation can call with its own guidelines, rather than copying the workflow around.
- Keep your workflows in Git and evolve them through pull requests, so the reviewer is itself code reviewed.
- Add support for GitLab or Bitbucket by swapping the fetch and post steps, keeping everything in between identical.
- Give teams their own reusable workflows, repositories or environments and access controls, so the security team can edit review guidelines without being able to change the workflow logic itself.
